...
uint32_t calculate_crc32(const void *data, uint32_t length);
...
//prhs are inputs, plhs are outputs
mxArray *data_in_m;
const mwSize *dims;
uint32_t *data_in;
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  //associate inputs
  data_in_m = mxDuplicateArray(prhs[0]);
  data_in = (uint32_t*) mxGetData(data_in_m);
  dims = mxGetDimensions(prhs[0]);
  int len = (int)dims[1]/4; //row vector
//   mexPrintf("%d\n", len);
//   for(int i=0; i<len; i++){
//      mexPrintf("%d\n", data_in[i]);
//   }
  ...
  for (int i = 0; i < len; i++){
    crc = CrcSTM32(crc, data_in[i]);
  }
//   mexPrintf("%d\n", crc);
  //associate output
  plhs[0] = mxCreateDoubleScalar(crc);
}
