...
retval_t init_ramp(adc_data_t* adc_data, dac_data_t* dac_data, uint16_t length,
...
retval_t set_ramp_length(adc_data_t* adc_data, dac_data_t* dac_data,
		uint16_t length, uint16_t fall_percent) {
	...
	dac_data->ramp = ramp;
	adc_data->size = 2*length;
	//the size must be twice the length of one channel, we convert two channels and double buffer (2*2*2)
	adc_data->data = pvPortMalloc(adc_data->size * 16 * sizeof(uint16_t));
	return RET_OK;
}
...
retval_t set_modulation(adc_data_t* adc_data, dac_data_t* dac_data) {
	if (check_modulation_ok() == RET_OK) {
		...
		adc_data->size = 2*temp_ramp.length;
		//the size must be twice the length of one channel, we convert two channels and double buffer (2*2*2)
		adc_data->data = pvPortMalloc(adc_data->size * 16 * sizeof(uint16_t));
		return RET_OK;
	}
...
retval_t start_acquisition(adc_data_t* adc_data, dac_data_t* dac_data) {
	...
	HAL_ADCEx_MultiModeStart_DMA(&hadc1, (uint32_t*) adc_data->data, adc_data->size*4); // size * 32bits * 2buffers *2samples
